...
async function main() {
  ...
  if (deferred.length === 0) {
    console.log('  ✅ No hay emails en estado DEFERRED');
  } else {
    deferred.forEach((job) => {
      ...
      console.log(`  Error: ${job.lastError ? job.lastError.substring(0, 80) : 'N/A'}...`);
  ...
  if (failed.length === 0) {
    ...
    failed.forEach((job) => {
      ...
      console.log(`  Destinatario: ${job.recipientEmail}`);
      console.log(`  Intentos: ${job.attemptCount}`);
      console.log(`  Error: ${job.lastError ? job.lastError.substring(0, 100) : 'N/A'}...`);
      console.log('');
    });
  }
  ...
  if (providers.length === 0) {
    ...
    providers.forEach((provider) => {
      ...
      console.log(`  Bloqueado hasta: ${provider.blockedUntil ? provider.blockedUntil.toISOString() : 'N/A'}`);
      console.log(`  Último error: ${provider.lastError ? provider.lastError.substring(0, 100) : 'N/A'}...`);
      console.log(`  Último uso: ${provider.lastUsedAt ? provider.lastUsedAt.toISOString() : 'N/A'}`);
      console.log('');
    });
  }

  console.log('\n⏱️ Próximos emails a procesar (PENDING):\n');
  ...
  if (pending.length === 0) {
    ...
    pending.forEach((job) => {
      ...
      console.log(`  Destinatario: ${job.recipientEmail}`);
      console.log(`  Disponible en: ${availableIn <= 0 ? 'AHORA' : availableIn + ' minutos'}`);
      console.log(`  Creado: ${job.createdAt.toISOString()}`);
      console.log('');
    });
  }
