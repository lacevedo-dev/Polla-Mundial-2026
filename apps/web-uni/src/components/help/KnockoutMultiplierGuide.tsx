...
export const KnockoutMultiplierGuide: React.FC<KnockoutMultiplierGuideProps> = ({
    ...
    return (
        <div
            className={`${isCompact ? 'space-y-3' : 'rounded-2xl border-2 border-sky-200 bg-sky-50 p-6 space-y-4'} ${className}`}
                            ...
                            {KNOCKOUT_MULTIPLIER_EXAMPLES.map((ex) => (
                                ...
                                >
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        ...
                                        <span
                                            className={`font-black tabular-nums ${'highlight' in ex && ex.highlight ? 'text-amber-700' : 'text-sky-700'} ${isCompact ? 'text-xs' : 'text-sm'}`}
                                        >
                                            {ex.total}
                                        </span>
                                    </div>
