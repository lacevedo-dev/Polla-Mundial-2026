...
describe('LeaguesService', () => {
    const createService = (prismaMock: any, participationMock: any = { createPrincipalObligationForInvitation: jest.fn() }) =>
        new LeaguesService(
            prismaMock as any,
            participationMock as any,
            { createInAppNotification: jest.fn() } as any,
            { isEnabled: jest.fn().mockReturnValue(false), sendWhatsApp: jest.fn(), sendSMS: jest.fn() } as any,
            { enqueueEmail: jest.fn() } as any,
            { enqueuePaymentReminder: jest.fn() } as any,
            { send: jest.fn().mockResolvedValue({ sent: false }) } as any,
