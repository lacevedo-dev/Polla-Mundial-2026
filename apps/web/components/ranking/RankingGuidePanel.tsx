    ...
    return (
        ...
        >
            <button
                ...
                className="w-full min-h-[2.75rem] px-3 py-2.5 sm:px-4 text-left hover:bg-slate-50/80 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-lime-400"
            >
                <div className="flex items-start gap-2 sm:gap-3">
                    <Info className="h-4 w-4 text-slate-400 shrink-0 mt-0.5" aria-hidden="true" />
                    <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-black uppercase tracking-wide text-slate-500">
                            Cómo leer los puntos
                        </p>
                        <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 leading-snug">
                            {expanded
                                ? 'Toca para ocultar la explicación'
                                : 'Iconos en cada fila · Toca para ver qué significa cada uno'}
                        </p>
                        ...
                        {!expanded && (
                            ...
                            >
                                {TIEBREAK_ROW_METRICS.map((metric, index) => (
                                    <span
                                        key={metric.id}
                                        className="inline-flex items-center justify-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-1.5 py-1 min-h-[1.75rem]"
                                    >
                                        <span className="text-xs leading-none">{metric.icon}</span>
                                        <span className="text-[8px] sm:text-[9px] font-bold text-slate-600 truncate">
                                            {metric.shortLabel}
                                        </span>
                                        <span className="text-[7px] font-black text-slate-300 tabular-nums">
                                            {index + 1}
                                        </span>
                                    </span>
                                ))}
            ...
            >
                ...
                <p className="text-[10px] text-slate-400 leading-snug">
                    Toca cualquier participante para ver el detalle partido a partido y los bonos aplicados.
                </p>
            </div>
        </section>
    );
