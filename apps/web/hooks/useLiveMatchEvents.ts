...
function mergeMatchEventsWithProfiles(
    primary: MatchEventItem[],
    enriched?: MatchEventItem[],
): MatchEventItem[] {
    if (!enriched?.length) return primary;

    const enrichedByKey = new Map(
        enriched.map((event) => [matchEventIdentityKey(event), event] as const),
    );

    return primary.map((event) => {
        const extra = enrichedByKey.get(matchEventIdentityKey(event));
        if (!extra) return event;

        const playerProfile =
            event.playerProfile?.photoUrl?.trim()
                ? event.playerProfile
                : extra.playerProfile ?? event.playerProfile;

        return {
            ...event,
            playerProfile,
            playerExternalId: event.playerExternalId ?? extra.playerExternalId ?? null,
            teamStickerTheme: event.teamStickerTheme ?? extra.teamStickerTheme ?? null,
        };
    });
}

function matchEventIdentityKey(event: MatchEventItem): string {
    return [
        event.type,
        event.minute,
        event.extraMin ?? '',
    ...
    const eventsByMatchId = React.useMemo(() => {
        ...
        for (const matchId of liveMatchIds) {
            ...
            if (fromSse && fromSse.length > 0) {
                merged.set(
                    matchId,
                    dedupeMatchEvents(mergeMatchEventsWithProfiles(fromSse, fromFetch)),
                );
