                ...
                return (
                  <article key={`${match.id}-mobile`} className={`rounded-[1.5rem] border-l-4 border border-slate-200 bg-white p-4 shadow-sm ${STATUS_CARD_BORDER[match.status] || 'border-l-slate-300'}`}>
                    ...
                    <div className="mt-4 grid grid-cols-7 gap-2">
                      ...
                      <button onClick={() => setEditMatch(match)} className="rounded-xl border border-amber-300 bg-amber-50 px-2 py-2 text-xs font-bold text-amber-700" aria-label={`Ajustar ${match.homeTeam.name} vs ${match.awayTeam.name}`} title="Ajustar partido (equipos, fecha, estado) y activar en pollas"><Edit3 size={14} className="mx-auto" /></button>
                      <button onClick={() => setScoreMatch(match)} className="rounded-xl border border-slate-200 px-2 py-2 text-xs font-bold text-slate-700" aria-label={`Actualizar resultado de ${match.homeTeam.name} vs ${match.awayTeam.name}`} title="Actualizar resultado / marcador"><Hash size={14} className="mx-auto" /></button>
                    ...
                    return (
                      <div key={match.id} className="grid grid-cols-[2fr_auto] items-center gap-4 px-5 py-3.5 transition-colors hover:bg-slate-50 md:grid-cols-[2fr_1fr_1.2fr_auto]">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => setEditMatch(match)} className="flex h-7 w-7 items-center justify-center rounded-lg border border-amber-200 bg-amber-50 text-amber-700 transition-all hover:bg-amber-100 hover:text-amber-800" title="Ajustar partido (equipos, fecha, estado) y activar en pollas"><Edit3 size={14} /></button>
                          <button onClick={() => setScoreMatch(match)} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 transition-all hover:bg-slate-100 hover:text-slate-700" title="Actualizar resultado / marcador"><Hash size={14} /></button>
