...
export class AuthService {
    ...
    async register(registerDto: RegisterDto, avatarFile?: AvatarUploadFile) {
        ...
        const expiresAt = calculateTokenExpiration(72);
        ...
        try {
            ...
            const appUrl = process.env.CORP_APP_URL || process.env.APP_URL || 'https://polla2026.com';
    ...
    private async verifyRecaptcha(token: string | undefined, expectedAction: string) {
        ...
        if (!token) {
            if (isRequired) {
                this.logger.warn('RECAPTCHA_REQUIRED=true pero el cliente no envió recaptchaToken');
