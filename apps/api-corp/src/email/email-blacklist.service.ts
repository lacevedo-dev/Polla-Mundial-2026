...
export class EmailBlacklistService {
  ...
  async isBlacklisted(email: string): Promise<BlacklistCheckResult> {
    ...
    const entry = await this.prisma.emailBlacklist.findUnique({
      ...
      select: { reason: true, blockedAt: true, notes: true },
    });

    const result: BlacklistCheckResult = entry
      ? { isBlacklisted: true, reason: entry.reason, blockedAt: entry.blockedAt, notes: entry.notes ?? undefined }
      : { isBlacklisted: false };
  ...
  async recordFailure(email: string, error: string): Promise<{ blocked: boolean; reason?: EmailBlacklistReason }> {
    const normalized = this.normalizeEmail(email);
    ...
    const existing = await this.prisma.emailBlacklist.findUnique({ where: { email: normalized } });

    if (existing) {
      const updated = await this.prisma.emailBlacklist.update({
        where: { email: normalized },
        data: { failureCount: { increment: 1 }, lastFailure: new Date(), lastError: error },
      });

      if (updated.failureCount >= EmailBlacklistService.AUTO_BLOCK_THRESHOLD) {
        this.logger.warn(`Email ${normalized} auto-blocked after ${updated.failureCount} failures`);
    ...
    await this.prisma.emailBlacklist.create({
      data: { email: normalized, reason: EmailBlacklistReason.REPEATED_FAILURE, failureCount: 1, lastFailure: new Date(), lastError: error, autoBlocked: false },
    });
    return { blocked: false };
  }

  async addToBlacklist(email: string, reason: EmailBlacklistReason, notes?: string, autoBlocked = false, blockedBy?: string): Promise<void> {
    const normalized = this.normalizeEmail(email);
    await this.prisma.emailBlacklist.upsert({
      where: { email: normalized },
      create: { email: normalized, reason, notes, autoBlocked, blockedBy },
      update: { reason, notes, blockedAt: new Date(), blockedBy },
    });
  ...
  async removeFromBlacklist(email: string): Promise<boolean> {
    const normalized = this.normalizeEmail(email);
    const deleted = await this.prisma.emailBlacklist.deleteMany({ where: { email: normalized } });
    if (deleted.count > 0) {
      this.invalidateCache();
  ...
  async listBlacklist(filters?: { reason?: EmailBlacklistReason; autoBlocked?: boolean; limit?: number }) {
    return this.prisma.emailBlacklist.findMany({
      where: {
        ...(filters?.reason ? { reason: filters.reason } : {}),
        ...(typeof filters?.autoBlocked === 'boolean' ? { autoBlocked: filters.autoBlocked } : {}),
      },
  ...
  async blockDomain(domain: string, reason: EmailBlacklistReason, blockedBy?: string): Promise<number> {
  ...
  private detectFailureReason(error: string): EmailBlacklistReason {
    for (const pattern of EmailBlacklistService.BOUNCE_PATTERNS) {
      if (pattern.test(error)) return EmailBlacklistReason.BOUNCE;
    }
    for (const pattern of EmailBlacklistService.SPAM_PATTERNS) {
      if (pattern.test(error)) return EmailBlacklistReason.SPAM_COMPLAINT;
    }
