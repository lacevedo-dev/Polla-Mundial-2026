...
export class EmailBacklogAuditService {
  ...
  async getAutomationStatus() {
    const [snapshot, lastRun, recentFailures] = await Promise.all([
      ...
      this.prisma.emailBacklogAuditRun.findFirst({ orderBy: { startedAt: 'desc' } }),
      this.prisma.emailBacklogAuditRun.count({
        where: { status: EmailBacklogAuditStatus.FAILED, startedAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } },
      }),
    ]);
    return { queue: snapshot, latestRun: lastRun, recentFailures };
  }
  ...
  async listRuns(page = 1, limit = 20) {
    ...
    const [total, runs] = await Promise.all([
      this.prisma.emailBacklogAuditRun.count(),
      this.prisma.emailBacklogAuditRun.findMany({ orderBy: { startedAt: 'desc' }, skip, take }),
    ]);
    return { total, page: Math.max(1, page), limit: take, runs };
  }
