...
export class EmailProviderAccountsService {
  ...
  async listAccounts(filters: ListEmailProviderAccountsInput = {}): Promise<AccountWithUsage[]> {
    ...
    const usages = await this.prisma.emailProviderUsage.findMany({
      where: {
        providerKey: { in: accounts.map((a) => a.key) },
        quotaWindowStart,
      },
    });

    const usageByKey = new Map(usages.map((u) => [u.providerKey, u]));
  ...
  async getAccountOrThrow(id: string): Promise<AccountWithUsage> {
    const quotaWindowStart = getQuotaWindowStart(new Date());
    const existing = await this.prisma.emailProviderAccount.findFirst({ where: { id, deletedAt: null } });
    if (!existing) throw new NotFoundException('Cuenta de correo no encontrada');

    const usage = await this.prisma.emailProviderUsage.findUnique({
      where: { providerKey_quotaWindowStart: { providerKey: existing.key, quotaWindowStart } },
    });

    return {
      ...existing,
      usageToday: usage ? { sentCount: usage.sentCount, blockedUntil: usage.blockedUntil, lastError: usage.lastError } : undefined,
    };
  }
  ...
  async updateAccount(id: string, input: UpdateEmailProviderAccountInput): Promise<AccountWithUsage> {
    ...
    const data: Prisma.EmailProviderAccountUpdateInput = {
      ...
      ...(input.reservedHighPriority != null ? { reservedHighPriority: sanitizeReserved(nextDailyLimit, input.reservedHighPriority) } : {}),
      ...(input.maxRecipientsPerMessage != null ? { maxRecipientsPerMessage: sanitizeInt(input.maxRecipientsPerMessage, existing.maxRecipientsPerMessage) } : {}),
      ...(input.maxEmailSizeMb != null ? { maxEmailSizeMb: sanitizeInt(input.maxEmailSizeMb, existing.maxEmailSizeMb) } : {}),
      ...(input.maxAttachmentSizeMb != null ? { maxAttachmentSizeMb: sanitizeInt(input.maxAttachmentSizeMb, existing.maxAttachmentSizeMb) } : {}),
      ...(input.active != null ? { active: input.active } : {}),
      ...(input.smtpPass !== undefined ? { smtpPassEncrypted: input.smtpPass ? this.cryptoService.encrypt(input.smtpPass.trim()) : null } : {}),
    };

    await this.prisma.emailProviderAccount.update({ where: { id }, data });
    return this.getAccountOrThrow(id);
  }

  async activateAccount(id: string): Promise<AccountWithUsage> { return this.updateAccount(id, { active: true }); }
  async deactivateAccount(id: string): Promise<AccountWithUsage> { return this.updateAccount(id, { active: false }); }

  async removeAccount(id: string): Promise<{ id: string; deletedAt: Date }> {
    const existing = await this.getAccountOrThrow(id);
    const deletedAt = new Date();
    await this.prisma.emailProviderAccount.update({ where: { id: existing.id }, data: { active: false, deletedAt } });
  ...
  async getProviderConfigs(): Promise<EmailProviderConfig[]> {
    ...
    return accounts.flatMap((account) => {
      try {
        ...
        this.logger.error(`Failed to decrypt password for provider ${account.key}: ${error instanceof Error ? error.message : String(error)}`);
        return [];
      }
    });
  }
  ...
  toAdminView(account: AccountWithUsage) {
    return {
      id: account.id, key: account.key, name: account.name, fromEmail: account.fromEmail,
      fromName: account.fromName, smtpHost: account.smtpHost, smtpPort: account.smtpPort,
      secure: account.secure, smtpUser: account.smtpUser, hasPassword: !!account.smtpPassEncrypted,
      dailyLimit: account.dailyLimit, reservedHighPriority: account.reservedHighPriority,
      maxRecipientsPerMessage: account.maxRecipientsPerMessage, maxEmailSizeMb: account.maxEmailSizeMb,
      maxAttachmentSizeMb: account.maxAttachmentSizeMb, active: account.active,
      blockedUntil: account.blockedUntil, lastUsedAt: account.lastUsedAt, lastError: account.lastError,
      deletedAt: account.deletedAt, createdAt: account.createdAt, updatedAt: account.updatedAt,
      usageToday: account.usageToday ?? { sentCount: 0, blockedUntil: null, lastError: null },
    };
  }
}

function normalizeKey(value: string): string { return value.trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '-'); }
function normalizeEmail(value: string): string { return value.trim().toLowerCase(); }
function sanitizeInt(value: number, fallback: number): number { const p = Number.isFinite(value) ? Math.trunc(value) : fallback; return p > 0 ? p : fallback; }
function sanitizeReserved(dailyLimit: number, reservedHighPriority: number | undefined): number { const s = sanitizeInt(dailyLimit, 100); return Math.min(s, sanitizeInt(reservedHighPriority ?? 60, 60)); }
